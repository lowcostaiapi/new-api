...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo.UpstreamHeaderTime.After(relayInfo.StartTime) {
		other["uhrt"] = float64(relayInfo.UpstreamHeaderTime.UnixMilli() - relayInfo.StartTime.UnixMilli())
	}
	...
	if trail, ok := ctx.Get("retry_errors"); ok {
		adminInfo["retry_errors"] = trail
	}
